...
mycursor.execute("CREATE TABLE rifa_lote(numero_rifa BIGINT, valor_rifa VARCHAR(50), id_pessoa BIGINT, data_reserva DATE, data_compensacao DATE, status_rifa TINYINT, CONSTRAINT numero_rifa_pk PRIMARY KEY (numero_rifa),  CONSTRAINT id_pessoa_fkey FOREIGN KEY (id_pessoa) REFERENCES Pessoa(id_pessoa), CONSTRAINT status_rifalote FOREIGN KEY (status_rifa) REFERENCES status_rifa(status))")
